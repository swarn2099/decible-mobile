...
export function DiscoveryGlassCard({ item, onPress, simplified = false }: Props) {
  ...
  const finderDisplay = item.finder_username
  ...
  const handleFinderPress = () => {
    if (item.finder_fan_id) {
      router.push(`/user/${item.finder_fan_id}`);
    }
  };
      ...
      >
        ...
        <View style={styles.glassStrip}>
          ...
          <View style={styles.stripContent}>
            ...
            {/* via @finder */}
            <Pressable onPress={handleFinderPress} style={styles.finderRow}>
              <Text style={styles.viaText}>via </Text>
              <Text
                style={[
                  styles.finderText,
                  item.finder_fan_id ? styles.finderLink : null,
                ]}
                numberOfLines={1}
              >
                {finderDisplay}
